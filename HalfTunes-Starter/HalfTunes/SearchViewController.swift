...
class SearchViewController: UIViewController {
  ...
  override func viewDidLoad() {
    super.viewDidLoad()
    _ = self.downloadsSession //ensures the app creates exactly one background session
...
//        let configuration = NSURLSessionConfiguration.defaultSessionConfiguration()
        //allow you to reference and “reconnect” to t
...
//    }
    ...
    //1
//    let download = Download(url:urlSt)
  }
  ...
  func resumeDownload(track: Track) {
    ...
    if let urlString = track.previewUrl,
        download = activeDownloads[urlString]{
            if let resumData = download.resumeData{
                download.downloadTask = downloadsSession.downloadTaskWithResumeData(resumData)
                download.downloadTask?.resume()
                download.isDownloading = true
...
//MAKR: NSURLSESSIONDOWNLOADDELEGATE
...
extension SearchViewController: UITableViewDataSource {
  ...
  func tableView(tableView: UITableView, cellForRowAtIndexPath indexPath: NSIndexPath) -> UITableViewCell {
    ...
    //download 
    let download = localFileExistsForTrack(track)
